...
pub fn compare_assertion(
    ...
    match operator {
        ...
        "greaterThan" | "lessThan" | "greaterThanOrEqual" | "lessThanOrEqual" => {
            ...
            (
                ...
                if passed {
                    ...
                    format!(
                        "Expected {} {} {} but got {}",
                        act_num, operator, exp_num, act_num
                    )
                },
            )
        }
        "matches" => {
            let exp = expected.unwrap_or("");
            let act = actual.unwrap_or("");
            let passed = regex::Regex::new(exp)
                .map(|re| re.is_match(act))
                .unwrap_or(false);
            (
                passed,
                if passed {
        ...
        "isJson" => {
            ...
            (
                passed,
                if passed {
                    "Value is valid JSON".into()
                } else {
                    "Value is not valid JSON".into()
                },
            )
        }
        "isType" => {
        ...
        _ => (false, format!("Unknown operator: {}", operator)),
    }
}
...
fn evaluate_schema_assertion(assertion: &Value, response: &ResponseData) -> (bool, String) {
    let Some(schema_str) = assertion["expected"].as_str() else {
        return (
            false,
            "No JSON Schema provided in expected field".to_string(),
        );
    };
