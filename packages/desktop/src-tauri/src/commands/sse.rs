    ...
    tokio::spawn(async move {
        ...
        'reconnect: loop {
            ...
            let (cancel_tx, mut cancel_rx) = tokio::sync::oneshot::channel::<()>();
            {
                ...
                log::debug!("SSE registering connection: '{}'", connection_id_clone);
                reg.insert(connection_id_clone.clone(), cancel_tx);
                let keys: Vec<String> = reg.keys().cloned().collect();
                log::debug!("SSE registry after insert: {:?}", keys);
            }
...
pub async fn sse_disconnect(
    ...
    let mut reg = registry.lock().await;
    let keys: Vec<String> = reg.keys().cloned().collect();
    log::debug!("SSE registry keys: {:?}", keys);

    if let Some(cancel) = reg.remove(&connection_id) {
        log::debug!("SSE cancel signal sent for '{}'", connection_id);
        let _ = cancel.send(());
    } else {
        log::debug!("SSE no connection found for '{}'", connection_id);
    }
