...
pub async fn open_project_dir(
    ...
    if let Some(folder) = picked {
        if let Some(path) = folder.as_path() {
            ...
            let projects =
                crate::services::storage::add_recent_project(&app_data_dir, &path_str, &dir_name)
                    .await
...
pub async fn remove_recent_project(
    ...
    let app_data_dir = app
        ...
        .app_data_dir()
        .map_err(|e| AppError::Other(format!("Failed to resolve app data dir: {}", e)))?;
    let projects = crate::services::storage::remove_recent_project(&app_data_dir, &path)
        .await
        .map_err(AppError::Storage)?;
    app.emit("recentProjectsLoaded", json!({ "data": projects }))
        .map_err(|e| AppError::Other(format!("Failed to emit recentProjectsLoaded: {}", e)))?;
...
pub async fn clear_recent_projects_cmd(app: tauri::AppHandle) -> Result<(), AppError> {
    let app_data_dir = app
        ...
        .app_data_dir()
        .map_err(|e| AppError::Other(format!("Failed to resolve app data dir: {}", e)))?;
    crate::services::storage::clear_recent_projects(&app_data_dir)
        .await
        .map_err(AppError::Storage)?;
...
pub async fn open_recent_project(
    ...
    if !path_buf.exists() {
        let app_data_dir = app
            ...
            .app_data_dir()
            .map_err(|e| AppError::Other(format!("Failed to resolve app data dir: {}", e)))?;
        let projects = crate::services::storage::remove_recent_project(&app_data_dir, &path_str)
            .await
            .map_err(AppError::Storage)?;
        app.emit("recentProjectsLoaded", json!({ "data": projects }))
...
pub async fn create_project(
    ...
    if let Some(folder) = picked {
        if let Some(path) = folder.as_path() {
            ...
            project_storage
                .save_collections(&serde_json::json!([]))
                .await
                .map_err(AppError::Storage)?;
            project_storage
                .save_environments(&serde_json::json!({
                    "environments": [],
                    "activeId": null
                }))
                .await
...
pub async fn get_workspace_meta(
    ...
    let meta = match path {
        Some(dir) => ProjectStorageService::new(dir)
            .load_workspace_meta()
            .await
            .map_err(AppError::Storage)?,
        None => None,
