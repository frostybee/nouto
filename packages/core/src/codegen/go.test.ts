...
describe('Go codegen', () => {
  ...
  it('should add comment for digest auth', () => {
    ...
    expect(code).toContain('third-party library');
  ...
  it('should add comment for AWS SigV4 auth', () => {
    ...
    expect(code).toContain('aws-sdk-go');
  ...
  it('should handle SSL client cert with LoadX509KeyPair', () => {
    const code = target.generate(makeRequest({
      ssl: { certPath: '/path/cert.pem', keyPath: '/path/key.pem' },
    }));
    expect(code).toContain('LoadX509KeyPair');
