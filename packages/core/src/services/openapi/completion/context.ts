/**
 * Key/value completion-context detection for OpenAPI documents. Pure and
 * host-agnostic: callers pass the raw text, the UTF-16 cursor offset, and a
 * pre-built pointer map (built once per request and shared across
 * completion/hover/code-action lookups). Hosts convert offsets to their own
 * position/range types (VS Code `Position`/`Range`, Monaco columns).
 */
import { getLocation } from 'jsonc-parser';
import { classifyPointer } from './registry';
