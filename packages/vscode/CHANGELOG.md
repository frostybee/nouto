...
- **Example validation**: `example` / `examples` values on schemas, media types, and parameters are validated against their schema and reported as the `example-invalid-schema` / `example-invalid-media` lint rules (Ajv on the extension host, honouring OpenAPI 3.0 `nullable`)
...
- **Opt-in lint rules** stay off until you pick a severity for them in Settings, so upgrades that add opt-in rules never turn them on silently
...
- **OpenAPI lint quick fixes**: lightbulb fixes for missing 4xx/5xx responses (adds a `default` response), unbounded string/array parameters (`maxLength`/`maxItems`), unconstrained `additionalProperties`, missing tags, missing `operationId` (derived from method and path), operations without security (require an existing scheme on the operation or globally), API keys in the query string (move to header), plaintext `http://` server URLs (switch to https), server URLs embedding credentials (strip userinfo), unused component schemas (remove), rate-limit header stubs on 2xx responses (opt-in rule), and derived operation summaries and an info description for the free-text rules; lint findings about something absent underline the owning key rather than the whole block, and policy-style rules are grouped separately in Settings
